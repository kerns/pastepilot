...
function activate(context) {
  console.log('Activating PastePilot X...');

  let disposable = vscode.commands.registerCommand('extension.pastePilot', async (uri, allUris) => {
    if (!uri || !allUris) {
    ...
    const globalConfig = vscode.workspace.getConfiguration('pastePilot');

    // Check if local configuration file exists
    const localConfigPath = path.join(vscode.workspace.workspaceFolders[0].uri.fsPath, '.pastepilot.config.js');
    ...
    // Success message
    const manageExtensionLink = new vscode.MarkdownString(`[Manage Extension](command:workbench.extensions.action.showExtension?%22kernsco.pastepilot%22)`);
